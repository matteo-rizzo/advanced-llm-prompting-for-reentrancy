...
LOGS_BASE_DIR = Path("logs", f"baseline_{timestamp}")
...
def process_contract(path_to_file: Path, gt_category: str, llm: LLMHandler, log_dir: Path) -> bool:
    ...
    try:
        if os.environ["USE_COT"] == "true":
            llm_response = llm.analyze_contract_with_cot(contract_content)
        else:
...
def evaluate(path_to_contracts: Path, model_name: str = None, files_to_process: list[str] = None) -> None:
    ...
    log_dir = LOGS_BASE_DIR / model_name / f"{gt_category}"
    try:
